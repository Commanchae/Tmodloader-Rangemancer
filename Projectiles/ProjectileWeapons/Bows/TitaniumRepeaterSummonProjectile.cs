    ...
    public class TitaniumRepeaterSummonProjectile : SummonProjectile
    {
        ...
        protected override void shoot(Item chosenAmmo, Vector2 position, Vector2 shootVel)
        {
            ...
            if (heldItem.ModItem is TitaniumRepeaterSummon)
            {
                ...
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), position, shootVel.RotatedBy(MathHelper.ToRadians(5)), chosenAmmo.shoot, Projectile.damage, Projectile.knockBack, Main.myPlayer);
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), position, shootVel.RotatedBy(MathHelper.ToRadians(-5)), chosenAmmo.shoot, Projectile.damage, Projectile.knockBack, Main.myPlayer);
