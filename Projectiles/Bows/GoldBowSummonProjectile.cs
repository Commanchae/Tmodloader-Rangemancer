        ...
        protected override void handleShot(Item chosenAmmo, Vector2 position, Vector2 shootVel, float angleOffset, Vector2 targetCenter)
        {
            ...
            if (heldItem.ModItem is GoldBowSummon)
            {
                ...
                if (specialShotCounter >= specialShotCooldown)
                {
                    ...
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position + new Vector2(0, Projectile.height / 2), Vector2.Transform(shootVel, Matrix.CreateRotationX(angleOffset)) * 2, ProjectileID.JestersArrow, (int)(Projectile.damage * 1.5f), Projectile.knockBack, Main.myPlayer);
